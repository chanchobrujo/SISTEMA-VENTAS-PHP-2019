...
<head>
	...
	<style type="text/css">
		*{
		...
		.contenido-modal,.contenido-modal2,.contenido-modal4,.contenido-modal3,.contenido-modal5{
			position: relative;
	...
	<?php 
		...
		if (isset($_POST['bventa'])) {
			...
			$sql = "INSERT INTO `venta`(`ID_VENTA`, `DNI_CLIENTE`, `FECHA`, `HORA`) 
			VALUES ('$idcenta','$dni',CURDATE(),curTime())";
			$insertar = mYsqli_query($con,$sql)or die("error: ".mysqli_error($con));

			for ($i=0; $i < $tot; $i++) {
			    $idp = $_SESSION['cod'][$i];
			    $can = $_SESSION['can'][$i];
			    $sql = "UPDATE `producto` SET `STOCK_PRODUCTO`=`STOCK_PRODUCTO` - '$can' 
			    WHERE ID_PRODUCTO='$idp'";
				$insertar = mYsqli_query($con,$sql)or die("error: ".mysqli_error($con));
				$sql = "INSERT INTO `dtl_venta`(ID_VENTA,ID_PRODUCTO,CANTIDAD) 
				VALUES ('$idcenta','$idp','$can')";
				$insertar = mYsqli_query($con,$sql)or die("error: ".mysqli_error($con));
			}
						...
						<div class="modal-body5">
							<form method="post">
								<table>
									<tr>
										<td>ID:</td>
										<td><input type="text" class="texto" name="idcenta"></td>
									</tr>
									<tr>
										<td>DNI:</td>
									...
									<tr>
										<td colspan="2">
											<center>
												<input type="submit" class="btn" name="bventa" value="ENVIAR">
											</center>
										</td>
									</tr>
								</table>
